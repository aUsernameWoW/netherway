...
public final class ModConfig {
    ...
    private ModConfig(CfgFile cfg, Path file, boolean loadedOk) {
        ...
        if (serverEnabled && !ServeCommand.supportsBackend(backendId)) {
            // 见 forge-1.7.10 同名逻辑：内置 serve 起不来、玩家 agent 也会拒绝，
            // 在拼错的地方说一次。
        ...
        if (rendezvousWanted && !serverRunAgent) {
            // 见 forge-1.7.10 同名逻辑：半开状态会下发「指向会合点却没有会合点」
            // 的凭证，比直接按关闭处理更害人。
        ...
        Map<String, String> params = parseParams(cfg.getStringList("params", "server",
                ...
                backendId);
        // sessionKey=auto：每次启动生成随机会话密钥——serve 与下发凭证同源，
        // 重启即轮换，玩家侧经缓存自愈闭环拿新密钥。
    ...
    public Credentials serverCredentials() {
        try {
            ...
            if (serverRendezvous && Credentials.BACKEND_GONC_P2P.equals(backendId)) {
                ...
                // the broker list gets the "origin" placeholder the client
