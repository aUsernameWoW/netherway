 ...
 *
 * <p>server.params 是通用的 key=value 列表而非某个隧道方案的专用字段——
 * 凭证就是「backend 标识 + 参数表」，配置也保持同构，将来换隧道方案时
...
public final class ModConfig {
    ...
    private ModConfig(LoadedConfiguration loaded) {
        ...
        backendId = cfg.getString("backend", "server", Credentials.BACKEND_GONC_P2P,
                L10n.tr("cfg.server.backend"));
        if (serverEnabled && !ServeCommand.supportsBackend(backendId)) {
            // Nothing downstream can use it: the built-in serve refuses to
            // start (serve.backendUnsupported) and players' agents answer
            // backend_unknown. Say so once, here, where the typo lives.
            LOG.warn(L10n.tr("config.unknownBackend", backendId, Credentials.BACKEND_GONC_P2P));
        }
        ...
        if (rendezvousWanted && !serverRunAgent) {
            // 光警告不够：serverCredentials 会据此注入 brokers=origin 占位，若这里
            // 仍按开启处理，就会下发「指向会合点却没有会合点」的凭证，玩家全员
            // 打洞失败，而服主看日志只会以为自己在会合点模式。所以直接按关闭处理。
            LOG.warn(L10n.tr("config.rendezvousNeedsRunAgent"));
        }
        // The rendezvous (the MQTT signaling broker) is embedded on loopback
        // behind the Minecraft port by the built-in serve.
        ...
        Map<String, String> params = parseParams(cfg.getStringList("params", "server",
                ...
                backendId);
        // sessionKey=auto：每次启动生成随机会话密钥，等于给玩家侧缓存的凭证上了
        ...
        // Forge 的 Configuration 会把配置里未列出的旧字段/拼错字段追加到传入列表。
    ...
    public Credentials serverCredentials() {
        try {
            ...
            if (serverRendezvous && Credentials.BACKEND_GONC_P2P.equals(backendId)) {
                // 内嵌会合点模式下地址由客户端自己补：会合点就在这台服务器的
                // Minecraft 端口后面，玩家知道自己连的是哪；服务端反而未必知道
                // 自己的公网入口（NAT 后、多入口、域名与实际入口不一致都常见）。
                // The broker list gets the "origin" placeholder that the client
                // resolves to the entry it came in through. An explicit
                // operator-supplied list is left untouched (external brokers
                // are their choice).
    ...
    /**
     * Whether the embedded rendezvous is on. The built-in serve then runs the
     * signaling broker on loopback instead of using public brokers, and
     * players' signaling connections are relayed in from the Minecraft port
     * by {@link ConnectionSniffer}.
     */
