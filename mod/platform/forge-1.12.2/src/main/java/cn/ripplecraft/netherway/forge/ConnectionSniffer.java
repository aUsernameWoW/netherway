...
final class ConnectionSniffer {
     ...
     * 连接最初的字节流过这里。
     *
     * <p>刻意不用 ByteToMessageDecoder，与 1.7.10 平台层保持同一写法：
     * 手动攒缓冲在 4.0/4.1 上行为完全一致，不必依赖各版本 decoder
     * 边角行为的差异。两个解析器都保证有界（预认证帧头 8 字节 +
    ...
    private static final class Sniffer extends ChannelInboundHandlerAdapter {
        ...
        private void connectRendezvous(final ChannelHandlerContext c) {
            ...
            // channel 类型必须与前端连接同族：Linux 上服务端默认启用 epoll
            // 原生 transport（use-native-transport），Nio channel 注册不进
            // epoll 的 event loop。accept 出来的 channel 自己的类恰好就是
            // 「与该 event loop 匹配的 SocketChannel 实现」，直接复用它。
         ...
         * 后者是 Netty 官方 HexDumpProxy 的写法，在 1.7.10 的 Netty 4.0.10 上
         * 实测会死锁（回环写同步完成，read() 被读循环结尾吞掉，几百 KB 即
         * 卡死）。4.1 上未必复现，但可写性驱动在两边都正确，各版本同一写法。
         */
        ...
        /**
         * 把真实来源写回 NetworkManager（pipeline 里名为 packet_handler 的那个）。
