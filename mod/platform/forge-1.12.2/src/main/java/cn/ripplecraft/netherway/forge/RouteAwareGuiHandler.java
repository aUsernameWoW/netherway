 ...
 *
 * <p>Forge 1.12.2 仍没有“即将连接服务器”事件；最小侵入的做法是只替换原版
 * {@link GuiMultiplayer}（其他 mod 的自定义子类原样放行）。接管两个动作：
 * 点击连接（覆写所有单击、回车和双击最终都会经过的 {@code connectToSelected()}）
 * 与列表的延迟探测（覆写 {@code getServerPinger()} 换上路由感知的 pinger）。
...
public final class RouteAwareGuiHandler {
    ...
    public void onGuiOpen(GuiOpenEvent event) {
        if (!routes.replacesEntries() || event.getGui() == null
                || event.getGui().getClass() != GuiMultiplayer.class) {
            return;
        }
        try {
            GuiScreen parent = ReflectionHelper.getPrivateValue(
                    GuiMultiplayer.class, (GuiMultiplayer) event.getGui(),
                    "parentScreen", "field_146798_g");
            event.setGui(new RouteAwareGuiMultiplayer(parent, routes, bridge));
    ...
    private static final class RouteAwareGuiMultiplayer extends GuiMultiplayer {
        ...
        public void connectToSelected() {
            ...
            try {
                ServerSelectionList selection = ReflectionHelper.getPrivateValue(
                        GuiMultiplayer.class, this, "serverListSelector", "field_146803_h");
                int index = selection == null ? -1 : selection.getSelected();
                ...
                original = ((ServerListEntryNormal) item).getServerData();
            } catch (RuntimeException e) {
            ...
            ServerData direct = new ServerData(original.serverName, route.localAddress(), false);
            direct.copyFrom(original);
        ...
        public ServerPinger getOldServerPinger() {
            if (routePinger == null) {
                routePinger = new RouteAwarePinger(super.getOldServerPinger(), routes, bridge);
            }
            return routePinger;
        }
        ...
        private void forceRepingOnRouteChange() {
            ...
            try {
                ServerList list = ReflectionHelper.getPrivateValue(
                        GuiMultiplayer.class, this, "savedServerList", "field_146804_i");
                if (list == null) {
                    return;
                }
                for (int i = 0; i < list.countServers(); i++) {
                    ServerData entry = list.getServerData(i);
                    if (entry != null && entry.pinged
     ...
     * 与原版行为合流。
     */
    private static final class RouteAwarePinger extends ServerPinger {

        /** 在 {@link #pingedVia} 里表示“这次探测没走路由”。 */
        private static final String DIRECT = "";

        private final ServerPinger vanilla;
        private final WarmupEntryRouter routes;
        private final ForgeClientBridge bridge;
        /** 真实条目 → 承接探测结果的临时副本。 */
        ...
        RouteAwarePinger(ServerPinger vanilla, WarmupEntryRouter routes,
                         ForgeClientBridge bridge) {
        ...
        @Override
        public void ping(ServerData real) throws UnknownHostException {
            ...
            ServerData temp = new ServerData(real.serverName, route.localAddress(), false);
            vanilla.ping(temp);
        ...
        void mirrorResults() {
            for (Map.Entry<ServerData, ServerData> pair : mirrors.entrySet()) {
                ...
                real.version = temp.version;
                real.playerList = temp.playerList;
                real.setBase64EncodedIconData(temp.getBase64EncodedIconData());
