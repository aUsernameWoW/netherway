...
游戏加载期间 `WarmupController` 就为各服务提前建联，并在服务器列表里各维护一个
直连条目——玩家可以直接选它进服，全程不经过中转。预认证可以有界并行，因为它
只是短 TCP 请求；真正的 NAT 打洞严格串行，避免多个 agent 争抢 UDP 映射。建成的
隧道则可以同时保持。凭证轮换只重建对应服务的隧道，其他服务不受影响。

这里不做收藏、排序、测速排名或自动选服：mod 只把每个已知服务的直连能力提供给
原生服务器列表，选择权仍属于玩家和游戏。
