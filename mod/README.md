...
从第二次启动起体验更进一步：下发过的凭证会按 Minecraft 服务入口缓存在本地，
游戏加载期间 `WarmupController` 就为各服务提前建联。隧道已经就绪时，玩家点击
原服务器条目会直接从本地隧道进服，不再经历登录后的断线重连；真实入口仍原样
保存在 `servers.dat`，供下次启动预取和失败回退。玩家在预热完成前已经进服时，
成功后仍会立即切换。预认证可以有界并行，因为它只是短 TCP 请求；真正的 NAT
打洞严格串行，避免多个 agent 争抢 UDP 映射。建成的隧道则可以同时保持。
凭证轮换只重建对应服务的隧道，其他服务不受影响。

这里不做收藏、排序、测速排名或自动选服：mod 只在玩家选定服务后解析连接路线。
关闭 `client.replaceServerEntries` 可改回为每个服务维护独立直连条目的模式。
...
`Credentials.dedupKey()`（backend + Minecraft 入口 + 房间名）去重；旧缓存没有
入口信息时才退回 legacy 键。这样不同服务即使共用 backend/room 也不会互相覆盖。
...
`SelfTest` 共 432 项，覆盖平台识别、JSON 转义、事件解析容错、凭证往返（含中文与
