...
同一台机器可以运行多个 Netherway 服务端。只要每个服务器拥有不同且稳定的公网 `host:port` 入口，它们可以继续使用相同的默认房间名；客户端会按服务器入口分别保存凭证和直连条目。
...
多人游戏列表中会为每个服务维护一个 `[P2P直连] <房间> (<入口>)` 条目。玩家点击哪个就连接哪个；Netherway 不排名、不自动选服。某个服务打不通时，它会独立退避并重试，不影响其他服务。玩家仍可选择原有中转条目进入服务器。
